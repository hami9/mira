...
> وضعیت فعلی: **فاز ۳ — افزونه وردپرس/ووکامرس** تکمیل و روی یک سایت ووکامرس واقعی (با Docker) نصب و تست شده. فازهای بعدی طبق نقشه راه پروژه پیاده‌سازی می‌شوند.
...
- **بعد embedding تغییر کرد (۱۵۳۶ → ۷۶۸):** جدول `knowledge_base_chunks` در فاز ۰ برای embedding مدل‌های OpenAI (بعد ۱۵۳۶) طراحی شده بود. چون پرووایدر واقعی این استقرار Gemini با مدل `text-embedding-004` (بعد ۷۶۸) است، یک مایگریشن جدید (`Phase4AiSchema`) ستون رو تغییر داد. جدول قبل از این فاز خالی بود، پس بدون از دست رفتن داده.
...
- **تست واقعی:** آپلود سند نمونه از داشبورد، پرسیدن سوال از ویجت که با ارجاع به همون سند پاسخ داده شه، تست handoff (هم با کلمه‌ی «اپراتور» و هم با آستانه اطمینان پایین)، و تست Copilot با یک مکالمه‌ی واقعی — همه با کانتینر واقعی، نه فقط بازبینی کد.
...
| ۴ | هوش مصنوعی (RAG، Copilot، handoff) | 🔄 در حال تست |
| ۵ | گزارش‌گیری | در انتظار |
